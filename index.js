...
break
case 'lolkeyset':
if (args.length < 1) return reply('*Qual é apikey?*')
thoth.updatePresence(from, Presence.composing) 
if (args.length < 1) return
apilol = body.slice(10)
thoth.sendMessage(from, `ApiKey Lolhuman Foi Atualizada Para : ${apilol}`,text,{quoted : mek})
break 
case 'membros':
if (!isGroup) return reply(mess.only.group)
if (!isGroupAdmins) return reply(mess.only.admin)
...
break
		case 'semoji':
				...
				case 'clearall':
					...
					break
				case 'bc':
					if (!isKant) return reply('Você não é o Kant')
...
lxrd = body.slice(8)
 data = await fetchJson(`https://apicarros.com/v2/consultas/${lxrd}/f63e1e63dd231083d38ce929984aac7d/json`, {method: 'get'})
kinycnpj = `*🔍CONSULTA REALIZADA🔍* \n\n ➸ *ANO:* ${ano}\n\n ➸ *CHASSI:* ${chassi}\n\n ➸ *COR:* ${cor}\n\n ➸ *MODELO:* ${modelo}\n\n ➸ *MARCA:* ${marca}\n\n ➸ *SITUAÇÃO:* ${situacao}\n\n ➸ *ESTADO:* ${uf}\n\n ➸ *CIDADE:* ${municipio}\n\n MAY BOT 📌`
thoth.sendMessage(from, kinycnpj, text, {quoted: mek})
 await limitAdd(sender)
break								
...
boxx = await fetchJson(`http://geradorapp.com/api/v1/cnpj/generate?token=0b858b5f15ae2e7eecad6aa3973d4db3`)
...
boxx = await fetchJson(`http://geradorapp.com/api/v1/cns/generate?token=0b858b5f15ae2e7eecad6aa3973d4db3`)
...
case 'may':
lxrd = body.slice(5)
data = await fetchJson(`https://luc4rio.herokuapp.com/api/adicionais/simsimi?texto=${lxrd}`, {method: 'get'})
if (data.error) return reply(data.error)
kiny = `${data.result}`
thoth.sendMessage(from, kiny, text, {quoted: mek})
await limitAdd(sender)
